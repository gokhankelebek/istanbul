...
const StructuredDataManager = ({ type, data, aggregate = false }) => {
  ...
  const generateRestaurantData = (data) => {
    return {
      ...
      servesCuisine: data.servesCuisine || ['Turkish', 'Mediterranean', 'Middle Eastern', 'Halal'],
      ...
      review: data.reviews || [
        {
          ...
          reviewBody: 'Best Turkish food in Las Vegas! The döner kebab is amazing.'
        }
      ]
    };
  };
  ...
  const generateProductData = (data) => {
    return {
      ...
      image: data.image || 'https://www.istanbullv.com/images/menu/doner-kebab.jpg',
      description: data.description || 'Traditional Turkish döner kebab with fresh vegetables and special sauce.',
  ...
  const generateEventData = (data) => {
    return {
      ...
      image: data.image || 'https://www.istanbullv.com/images/events/turkish-night.jpg',
      description: data.description || 'Join us for a special Turkish Night featuring live music, traditional dance performances, and a special menu.',
      offers: {
        '@type': 'Offer',
        url: data.offers?.url || 'https://www.istanbullv.com/events/turkish-night',
  ...
  const generateRecipeData = (data) => {
    return {
      ...
      image: data.image || 'https://www.istanbullv.com/images/recipes/baklava.jpg',
      author: {
        '@type': 'Person',
        name: data.author?.name || 'Chef Mehmet'
      },
      datePublished: data.datePublished || '2023-02-15',
      description: data.description || 'Learn how to make authentic Turkish baklava with this traditional recipe.',
  ...
  const generateWebsiteData = (data) => {
    return {
      ...
      potentialAction: {
        ...
        target: {
          '@type': 'EntryPoint',
          urlTemplate: data.searchUrlTemplate || 'https://www.istanbullv.com/search?q={search_term_string}'
        },
