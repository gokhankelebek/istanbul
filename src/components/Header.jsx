    ...
    <header className="sticky top-0 z-50 w-full bg-istanbulRed/90 backdrop-blur shadow-lg rounded-b-xl">
      <nav className="flex items-center justify-between px-6 py-3 max-w-7xl mx-auto">
        ...
        <div className="hidden md:flex items-center space-x-6">
          <Link to="/" className="relative px-2 py-1 font-medium transition-colors duration-200 hover:text-istanbulRed-light after:block after:h-0.5 after:bg-offwhite after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-200 after:origin-left">{t('common.home')}</Link>
      ...
      {menuOpen && (
        <div className="md:hidden bg-istanbulRed/95 backdrop-blur-sm px-6 py-4 flex flex-col space-y-4 shadow-lg rounded-b-xl animate-fadeIn">
          <Link to="/" className="font-medium text-lg hover:text-istanbulRed-light" onClick={()=>setMenuOpen(false)}>{t('common.home')}</Link>
