...
function validateSourceContent() {
  ...
  const sourceFiles = fs.readdirSync(CONTENT_BLOG).filter(file => file.endsWith('.md'));
  ...
  sourceFiles.forEach(file => {
    ...
    try {
      ...
      if (!parsed.content || parsed.content.trim().length < 100) {
        logWarning(`${file}: Content seems too short (${parsed.content.length} chars)`);
      }
...
function validateSynchronization() {
  ...
  const publicContentFiles = fs.readdirSync(PUBLIC_CONTENT).filter(file => file.endsWith('.md'));
  
  if (sourceFiles.length !== publicContentFiles.length) {
    logError(`File count mismatch: Source (${sourceFiles.length}) vs Public Content (${publicContentFiles.length})`);
  }
  
  // Check each source file exists in public
  sourceFiles.forEach(file => {
    const publicPath = path.join(PUBLIC_CONTENT, file);
    const rootPath = path.join(PUBLIC_ROOT, file);
    ...
    if (!fs.existsSync(rootPath)) {
      logError(`Missing in public root: ${file}`);
    }
  });
...
function validateMetadata() {
  ...
  try {
    ...
    console.log(`📊 Found ${postsData.length} posts in JSON, ${sourceFiles.length} markdown files`);
    
    if (postsData.length !== sourceFiles.length) {
      logWarning(`Post count mismatch: JSON (${postsData.length}) vs Markdown (${sourceFiles.length})`);
    }
