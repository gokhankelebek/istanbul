...
const STATIC_ROUTES = [
  ...
  { path: "", priority: PRIORITIES.home, changefreq: CHANGE_FREQ.home },
  {
    ...
    changefreq: CHANGE_FREQ.mainPages,
  },
  {
    path: "blog-posts",
    priority: PRIORITIES.mainPages,
...
function generateSitemap() {
  ...
  STATIC_ROUTES.forEach((route) => {
    allUrls.push(
      generateUrlEntry(route.path, route.priority, route.changefreq, today)
    );
  });
  ...
  blogPosts.forEach((post) => {
    allUrls.push(
      generateUrlEntry(post.path, post.priority, post.changefreq, today)
    );
  });
  ...
  menuItems.forEach((item) => {
    allUrls.push(
      generateUrlEntry(item.path, item.priority, item.changefreq, today)
    );
  });
